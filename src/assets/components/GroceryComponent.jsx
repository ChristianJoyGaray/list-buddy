    ...
    <div className="grocery-buddy">
      ...
      <div className='input-section'>
        <div className='input-container'>
            <input 
            ref={inputRef}
            type='text' placeholder='Enter an item...' value={item} onChange={(event) => setItem(event.target.value)}/>
      ...
      <ul className='grocery-list'>
        {groceryItems.map((item) => <GroceryItemComponent key={item.id} item={item} 
        handleEditItem={handleEditItem}
        handleDeleteItem={handleDeleteItem}
        />)}
      </ul>
      {groceryItems.length > 0 ? 
      <button onClick={handleClearItems} className='btn-clear'>Clear Grocery Items</button> : null}
    </div>
  )
}
